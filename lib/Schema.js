...
const Schema = function (modelDef, contractSP, web3provider) {
      ...
      if (modelDefinition.properties[index].type === 'enum') {
        if (!modelDefinition.properties[index].values) {
          throw new TypeError(
            'Property at index ' +
              index +
              " is missing 'values' property which is mandatory for fields of type enum.",
          )
        }
        if (!modelDefinition.properties[index].defaultValue) {
          throw new TypeError(
            'Property at index ' +
              index +
              " is missing 'defaultValue' property which is mandatory for fields of type enum.",
          )
        }
        if (
          !modelDefinition.properties[index].values.includes(
            modelDefinition.properties[index].defaultValue,
          )
        ) {
          throw new TypeError(
            'Property at index ' +
              index +
              " 'defaultValue' is not included in the set of 'values'. The defaultValue of an enum type must be included in the set of values.",
          )
        }
        const valuesSet = new Set(modelDefinition.properties[index].values)
        if (
          valuesSet.length !== modelDefinition.properties[index].values.length
        ) {
          throw new TypeError(
            'Property at index ' +
              index +
              " 'values' must contain only unique values. It seems duplicate values exist",
          )
        }
      }
  ...
  function generateContract(modelDefinition, contractSavePath, callback) {
    ...
    fs.writeFile(
      contractSavePath + '/' + modelDefinition.contractName + '.sol',
      contractResult,
      function (err) {
        if (err) {
  ...
  this.deploy = function (callback) {
    if (isDeployed) {
      return callback(
        new Error('Model ' + name + ' is already deployed.'),
        false,
      )
    }
    const self = this
    generateContract(modelDefinition, contractSavePath, function (
      err,
      generatedContract,
    ) {
      if (!err) {
        const inputContract = fs
          .readFileSync(generatedContract.contract)
          .toString()
        const input = {
          language: 'Solidity',
          sources: {},
        ...
        const bytecode =
          output.contracts[fn][modelDefinition.contractName].evm.bytecode.object
  ...
  this.save = function (newValue, callback) {
    ...
    if (Object.keys(newValue).toString() !== propertyNames.toString()) {
      return callback(
        new Error('Instance does not match Schema definition'),
        undefined,
      )
    }
    const idToAdd = preprocessId(newValue[idField])
    contractDeployed.methods
      .addRecord(JSON.stringify(newValue), idToAdd)
      .send(transactionOptions)
      .then(function () {
  ...
  this.updateById = function (Id, newValue, callback) {
    ...
    contractDeployed.methods
      .updateRecord(idToLook, JSON.stringify(newValue))
      .send(transactionOptions)
      .then(function () {
        return callback(null, newValue)
      })
