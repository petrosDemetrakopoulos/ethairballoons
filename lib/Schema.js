...
var Schema = function (modelDef, contractSP, web3provider) {
	...
	this.deleteById = function (Id, callback) {
		var idToLook = preprocessId(Id);
		if (contractDeployed) {
			contractDeployed.methods.deleteRecord(idToLook).send({
				from: account,
				gas: 150000000,
				gasPrice: '30000000000000'
			}).then(function () {
	...
	this.updateById = function(Id, newValue, callback) {
		var idToLook = preprocessId(Id);
		if (contractDeployed) {
			contractDeployed.methods.updateRecord(idToLook,JSON.stringify(newValue)).send({
				from: account,
				gas: 150000000,
				gasPrice: '30000000000000'
			}).then(function () {
