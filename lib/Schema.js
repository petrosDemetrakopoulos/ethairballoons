...
const Schema = function(modelDef, contractSP, web3provider) {
    ...
    let contractDeployed = undefined;
    ...
    this.deploy = function(callback) {
        ...
        generateContract(modelDefinition, contractSavePath, function(err, generatedContract) {
            if (!err) {
                let inputContract = fs.readFileSync(generatedContract.contract).toString();
                let input = {
                    language: 'Solidity',
                    sources: {},
                    settings: {
                ...
                let abi = output.contracts[fn][modelDefinition.contractName].abi;
    ...
    this.save = function(newValue, callback) {
        ...
        let idToAdd = preprocessId(newValue[idField]);
